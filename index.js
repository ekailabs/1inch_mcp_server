...
const makerPrivateKey = "Enter Wallet Private Key";
const makerAddress = "Enter Wallet Address";
const devPortalApiKey = "Enter 1inch Portal API Key";
...
const networkConfigs = {
    ...
    [NetworkEnum.ARBITRUM]: "https://arb1.arbitrum.io/rpc" // Handle NetworkEnum value
};
...
async function executeCrossChainSwap({
    ...
    try {
        // Create provider based on source chain
        const nodeUrl = networkConfigs[srcChainId];
        console.error('nodeUrl', nodeUrl);
        ...
        // Try multiple possible property names for order hash
        const orderHash = quoteResponse?.orderHash ||
            quoteResponse?.hash ||
            quoteResponse?.id ||
            quoteResponse?.orderId ||
            quoteResponse?.transactionHash ||
            `order_${Date.now()}`; // Generate fallback ID
